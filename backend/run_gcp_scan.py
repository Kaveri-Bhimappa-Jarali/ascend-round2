#!/usr/bin/env python
"""
Utility script to run the GCP Cloud configuration scan independently.
"""
import sys
import os
import json
...
def main():
    parser = argparse.ArgumentParser(description="Run the CloudCompliance Sentinel GCP Resource Collector.")
    parser.add_argument("--demo", action="store_true", help="Force run in Mock Demo Mode.")
    parser.add_argument("--real", action="store_true", help="Force run in Real Cloud Mode (requires GCP credentials).")
    ...
    print("=" * 60)
    print("CloudCompliance Sentinel - GCP Scanner CLI")
    print("=" * 60)
    
    try:
        collector = GCPCollector(demo_mode=demo_mode)
